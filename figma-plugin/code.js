...
const numberOrMixed = (value) => (isMixed(value) ? 'mixed' : finiteNumber(value));
...
function serializePaints(value) {
...
function serializeDesignEvidence(node, depth = 0) {
    const sized = node;
    const bounds = 'absoluteBoundingBox' in node ? node.absoluteBoundingBox : null;
    const children = 'children' in node && depth < 5
        ? node.children.slice(0, 80).map((child) => serializeDesignEvidence(child, depth + 1))
        : undefined;
    const evidence = {
        id: node.id,
        name: node.name,
...
function serializeSelection(node) {
    const sized = node;
    return {
        id: node.id,
        name: node.name,
